...
data_direct <- "../data/hepb_mortality3.csv" 
census_data <- "../data/census_regions.xls"

setwd("../tables") ## the outputs are saved here
...
create_flextable <- function(sum1,title1) {
  ...
  ft <- ft %>% set_header_labels(ft,
                                 Var = "",
                                 Mean1 = "2011-2017", Mean1.1 = "2018-2019", Diff1 = "",
                                 Mean2 = "2011-2017", Mean2.1 = "2018-2019", Diff2 = "",
                                 Mean3 = "2011-2017", Mean3.1 = "2018-2019", Diff3 = "")
...
create_flextable2 <- function(sum11,title1) {
  ...
  ft <- ft %>% set_header_labels(ft,
                                 ...
                                 Mean2 = "2011-2017", Mean2.1 = "2018-2019", Diff2 = "",
                                 Mean3 = "2011-2017", Mean3.1 = "2018-2019", Diff3 = "",
                                 Mean4 = "2011-2017", Mean4.1 = "2018-2019", Diff4 = "")
...
create_models <- function(data, dept_var) {
  ...
  model2 <- feols(.[dept_var] ~ .[endo] | fips + age_fe, data = data)
  model3 <- feols(.[dept_var] ~ .[c(endo, exog)] | fips + age_fe, data = data)
...
print_reg2 <- function(latex_file_name1, title1, data_direct) {
  ...
  df <- tryCatch(
    {
      read_csv(data_direct) %>% 
        filter(vax_type == "Hep B" & age < 3)
    },
    error = function(e) {
      stop("Error reading or filtering data: ", e$message)
    }
  )
...
print_reg5 <- function(latex_file_name1, title1, data_direct) {
  ...
  exog <<- c("low_birthweight_raw_value",
             "children_in_poverty_raw_value",
             "percent_females_raw_value",
             "adult_smoking_raw_value",
             "median_household_income_raw_value",
  ...
  model2 <- feols(.[dept_var] ~.[c(endo)] | fips + age_fe, data = df1)
  model02 = summary(model2, cluster = c('fips'))
  
  model3 <- feols(.[dept_var] ~.[c(endo,exog)] | fips + age_fe, data = df1)
  ...
  model5 <- feols(.[dept_var] ~.[c(endo)] | fips + age_fe, data = df2)
  model05 = summary(model5, cluster = c('fips'))
  
  model6 <- feols(.[dept_var] ~.[c(endo,exog)] | fips + age_fe, data = df2)
  ...
  model8 <- feols(.[dept_var] ~.[c(endo)] | fips + age_fe, data = df3)
  model08 = summary(model8, cluster = c('fips'))
  
  model9 <- feols(.[dept_var] ~.[c(endo,exog)] | fips + age_fe, data = df3)
...
df <- read_csv( "../data/hepb_mortality3.csv") %>% filter(year<2016) 
...
print_reg2_placebo <- function(latex_file_name1, title1) {
  ...
  models <- list(
    ...
    create_model(df %>% rename(vax_le4d = vax_est), "vax_le4d", endo, c("fips", "age")),
    ...
    create_model(df %>% filter(age < 2) %>% rename(vax_le3d = vax_est), "vax_le3d", endo, c("fips", "age")),
    ...
    create_model(df %>% filter(age < 1) %>% rename(vax_le2d = vax_est), "vax_le2d", endo, c("fips", "age")),
...
print_reg3 <- function(latex_file_name1, title1) {
  ...
  models <- list(
    ...
    create_model(df %>% rename(mort_le4d = mortality_rate), "mort_le4d", endo, c("fips", "age_fe")),
    ...
    create_model(df %>% filter(age < 2) %>% rename(mort_le3d = mortality_rate), "mort_le3d", endo, c("fips", "age_fe")),
    ...
    create_model(df %>% filter(age < 1) %>% rename(mort_le2d = mortality_rate), "mort_le2d", endo, c("fips", "age_fe")),
